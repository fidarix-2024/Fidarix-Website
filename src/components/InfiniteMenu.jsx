  ...
  return (
    <div className="im-container">
      <canvas
        ...
        ref={canvasRef}
        className={`im-canvas ${isMoving ? 'grabbing' : 'grab'}`}
      />

      {activeItem && (
        <>
          <h2 className={`im-title ${isMoving ? 'hidden' : 'visible'}`}>
            {activeItem.title}
          </h2>

          <p className={`im-description ${isMoving ? 'hidden' : 'visible'}`}>
            {activeItem.description}
          </p>

          <div
            onClick={handleButtonClick}
            className={`im-button ${isMoving ? 'hidden-button' : 'visible-button'}`}
          >
            <p className="im-button-icon">&#x2197;</p>
          </div>
        </>
      )}
    </div>
